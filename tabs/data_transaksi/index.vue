<template>
  <div class="w-full h-full flex flex-col border-t-4 border-slate-600">
...
const tabs_form = () => markRaw(defineAsyncComponent(() => import('~/tabs/data_transaksi/form.vue')));

const form_add = async() => {
  await useCommonStore().setDynamicComponent(tabs_form,"Tambah Transaksi",{id:null});
...
const form_edit = async() => {
  if (selected.value == -1) {
    display({ show: true, status: "Failed", message: "Silahkan Pilih Data Terlebih Dahulu" });
  } else {
    await useCommonStore().setDynamicComponent(tabs_form,"Ubah Transaksi",{id: transactions.value[selected.value].id});
    // router.push({ name: 'data_transaksi-form', query: { id: transactions.value[selected.value].id } });
  }
};

const tabs_confirm = () => markRaw(defineAsyncComponent(() => import('~/tabs/data_transaksi/confirm.vue')));

const checkAndConfirm = async() => {
  if (selected.value == -1) {
    display({ show: true, status: "Failed", message: "Silahkan Pilih Data Terlebih Dahulu" });
  } else {
    await useCommonStore().setDynamicComponent(tabs_confirm,"Konfirmasi Transaksi",{id: transactions.value[selected.value].id});

    // router.push({ name: 'data_transaksi-confirm', query: { id: transactions.value[selected.value].id } });
  }
};

const tabs_request = () => markRaw(defineAsyncComponent(() => import('~/tabs/data_transaksi/request.vue')));

const goToRequest = async() => {
  await useCommonStore().setDynamicComponent(tabs_request,"List Transaksi");

  // router.push({ name: 'data_transaksi-request'});
};

const tabs_summary= () => markRaw(defineAsyncComponent(() => import('~/tabs/data_transaksi/summary.vue')));

const openSummary = async() => {
  await useCommonStore().setDynamicComponent(tabs_summary,"Summary Transaksi");
  // router.push({ name: 'data_transaksi-request'});
};
